...
class TestAcmeRPCMicroService:
    ...
    def test_square_odd_numbers(self):
        ...
        squared_odd_numbers = json.loads(
                acme_rpc_microservice.square_odd_numbers(items))
        assert squared_odd_numbers == [1, 9, 25, 49, 81]

    def test_huffman_encode_strings(self):
        items = ["Acme Bricks", "Contoso.com"]
        encoded_strings = json.loads(
                acme_rpc_microservice.huffman_encode_strings(items))
    ...
    def test_decode_huffman_encoded_string(self):
        ...
        decoded_string = json.loads(
                acme_rpc_microservice.decode_huffman_encoded_string(
                encoded_string))
